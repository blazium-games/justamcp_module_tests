...
func test_autowork_directory_and_script_tools() -> void:
	...
	assert_true(_error_text(by_script_validation).contains("already running"), "run_test_script should explain nested execution was blocked")
	...
	assert_true(_error_text(by_directory_validation).contains("already running"), "run_tests_in_directory should explain nested execution was blocked")
